...
async function main(){
  const itemId='168494447539';
  const mappings=[{catalogItemId:19928, qty:36}];
  ...
  console.log('mapped', itemId, JSON.stringify((await sql`SELECT mappings FROM ebay_listing_mappings WHERE ebay_item_id=${itemId}`)[0].mappings));
  await sql.end();
}
main();
