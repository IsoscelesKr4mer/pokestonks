...
async function main() {
  ...
  >`
    WITH purchase_remaining AS (
      SELECT p.catalog_item_id,
             ...
             MIN(p.cost_cents)::int AS per_unit_cost_cents
      FROM purchases p
    ...
    SELECT c.id AS catalog_item_id, c.name,
           pr.qty_held, pr.per_unit_cost_cents,
    ...
    WHERE c.name ILIKE '%zacian%'
       OR c.name ILIKE '%hop%'
       OR c.name ILIKE '%black bolt%elite%'
    ORDER BY c.name;
  `;

  console.log('Hop / Zacian / BB ETB:');
  for (const it of items) {
    ...
    console.log(`[${it.catalog_item_id}] ${it.name}  qty=${it.qty_held ?? 0}  cost/u=${cost}  market=${mkt}`);
  }
