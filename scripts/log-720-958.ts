...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, 17236, '2026-07-20', 1, 500, 'Vending Machine', 'Edmonds Safeway 9:58 - only a DR pack dropped (was hoping for last WF/BB bundle); assumed bought')`;
  console.log(`Destined Rivals pack: held ${b} -> ${await held(17236)}`);
  appendFileSync('data/drop_log.csv',
    '2026-07-20,Monday,21:58,Edmonds Safeway,hit,Destined Rivals Booster Pack,1,5.00,last-chance WF/BB run; only a DR dropped - assumed bought (confirm if left)\n');
