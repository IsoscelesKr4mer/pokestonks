...
const ORDER_ID = '09-14959-92118';
const SALE_DATE = '2026-07-28'; // order 2026-07-29T00:26Z = 5:26pm PDT Tue 7/28
const FEES = 2446; // totalMarketplaceFee $24.46 (shipping is a wash; revenue = item subtotal only)
const NOTES = `eBay order #${ORDER_ID} (twofer: Prismatic + DR bundle)`;
const LINES: [number, { c: number; q: number }[]][] = [
  [15999, [{ c: 19776, q: 1 }, { c: 17235, q: 1 }]], // DRPRIS-TWOFER = 1 Prismatic bundle + 1 DR bundle
];

async function mkt(id: number) { const r = await sql`SELECT market_price_cents m FROM market_prices WHERE catalog_item_id=${id} ORDER BY snapshot_date DESC LIMIT 1`; return r[0]?.m ?? 0; }
...
function matchFifo(lots: any[], totalQty: number, totalSale: number, totalFees: number) {
  ...
  for (const l of sorted) { if (!rem) break; const take = Math.min(rem, l.qtyAvailable); pend.push({ purchaseId: l.purchaseId, quantity: take, matchedCostCents: take * l.costCents }); rem -= take; }
  const rows = pend.map(p => ({ purchaseId: p.purchaseId, quantity: p.quantity, salePriceCents: Math.floor(totalSale * p.quantity / totalQty), feesCents: Math.floor(totalFees * p.quantity / totalQty), matchedCostCents: p.matchedCostCents }));
...
async function main() {
  ...
  const cats = [...new Set(LINES.flatMap(([, m]) => m.map(x => x.c)))];
  const px: Record<number, number> = {}; for (const c of cats) px[c] = await mkt(c);
  const perCat = new Map<number, { qty: number; rev: number }>();
  for (const [rev, map] of LINES) {
    const lw = map.reduce((a, m) => a + px[m.c] * m.q, 0);
    for (const m of map) { const share = lw > 0 ? (px[m.c] * m.q) / lw : m.q / map.reduce((a, e) => a + e.q, 0); const r = Math.round(rev * share); const e = perCat.get(m.c); if (e) { e.qty += m.q; e.rev += r; } else perCat.set(m.c, { qty: m.q, rev: r }); }
  }
  const ids = [...perCat.keys()]; const totalRev = [...perCat.values()].reduce((a, v) => a + v.rev, 0);
  let alloc = 0; const items = ids.map((cid, i) => { const v = perCat.get(cid)!; const fee = i === ids.length - 1 ? FEES - alloc : (totalRev > 0 ? Math.round(v.rev / totalRev * FEES) : Math.round(FEES / ids.length)); alloc += fee; return { catalogItemId: cid, quantity: v.qty, salePriceCents: v.rev, feesCents: Math.max(0, fee) }; });
  console.log('proposed items:', JSON.stringify(items));
  const uid = (await sql`SELECT user_id FROM purchases WHERE catalog_item_id=19776 AND deleted_at IS NULL LIMIT 1`)[0].user_id;
  const sgid = randomUUID(); const allRows: any[] = [];
  for (const it of items) { const rows = matchFifo(await openLots(it.catalogItemId), it.quantity, it.salePriceCents, it.feesCents); for (const r of rows) allRows.push({ ...r, catalogItemId: it.catalogItemId }); }
  await sql.begin(async (tx) => {
    for (const r of allRows) await tx`INSERT INTO sales (user_id, sale_group_id, purchase_id, sale_date, quantity, sale_price_cents, fees_cents, matched_cost_cents, platform, notes)
  ...
  console.log(`inserted ${allRows.length} sale rows, group ${sgid}`);
