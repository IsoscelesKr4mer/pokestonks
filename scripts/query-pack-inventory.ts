...
async function main() {
  ...
  >`
    ...
    WHERE (c.name ILIKE '%mega evolution%booster pack%'
        OR c.name ILIKE '%destined rivals%booster pack%')
      AND c.name NOT ILIKE '%sleeved%'
      AND c.name NOT ILIKE '%bundle%'
      AND c.name NOT ILIKE '%box%'
    ORDER BY c.name;
  `;

  console.log('ME / DR booster pack inventory:');
