...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, 17236, '2026-07-14', 2, 500, 'Vending Machine', 'Edmonds Safeway 1:58 drop - 2 DR packs')`;
  console.log(`Destined Rivals pack: held ${drB} -> ${await held(17236)}`);
  appendFileSync('data/drop_log.csv',
    '2026-07-14,Tuesday,13:58,Edmonds Safeway,hit,Destined Rivals Booster Pack,2,5.00,two DR packs at the 1:58 drop\n');
  console.log('drop_log +1 hit (2x DR at 13:58)');
  await sql.end();
}
