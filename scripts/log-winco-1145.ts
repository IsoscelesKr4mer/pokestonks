...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, ${id}, '2026-07-12', 1, 500, 'Vending Machine', 'Winco 11:45 drop')`;
  console.log(`Destined Rivals pack: held ${before} -> ${await held(id)}`);
  appendFileSync('data/drop_log.csv','2026-07-12,Sunday,11:45,Winco,hit,Destined Rivals Booster Pack,1,5.00,bought (Winco time confirmed :15/:45)\n');
  console.log('drop_log +1 hit (Winco)');
  await sql.end();
}
