...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, 17236, '2026-07-15', 2, 500, 'Vending Machine', 'Edmonds Safeway 11:28 - first trip of the day, 2 DR packs')`;
  console.log(`Destined Rivals pack: held ${b} -> ${await held(17236)}`);
  appendFileSync('data/drop_log.csv',
    '2026-07-15,Wednesday,11:28,Edmonds Safeway,hit,Destined Rivals Booster Pack,2,5.00,first trip of the day\n');
  console.log('drop_log +1 hit (2x DR at 11:28)');
  await sql.end();
}
