...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, ${id}, '2026-07-11', 1, 500, 'Vending Machine', 'Edmonds Safeway 2:58 drop')`;
  console.log(`Destined Rivals pack: held ${before} -> ${await held(id)}`);
  appendFileSync('data/drop_log.csv',
    '2026-07-11,Saturday,14:58,Edmonds Safeway,hit,Destined Rivals Booster Pack,1,5.00,bought\n'+
    '2026-07-11,Saturday,14:58,Edmonds Safeway,seen,Perfect Order Booster Pack,0,,left it\n');
