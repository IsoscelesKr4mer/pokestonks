...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, 17236, '2026-07-18', 1, 500, 'Vending Machine', 'Shoreline Fred Meyer 12:16 - 1 DR pack on the mark')`;
  console.log(`Destined Rivals pack: held ${b} -> ${await held(17236)}`);
  appendFileSync('data/drop_log.csv',
    '2026-07-18,Saturday,12:16,Shoreline Fred Meyer,hit,Destined Rivals Booster Pack,1,5.00,bought on the :16 mark\n');
  console.log('drop_log +1 hit (DR at 12:16 Fred Meyer)');
  await sql.end();
}
