...
async function main(){
  ...
  await sql`INSERT INTO purchases (user_id, catalog_item_id, purchase_date, quantity, cost_cents, source, notes)
    VALUES (${uid}, ${id}, '2026-07-13', 1, 5500, 'Zulu (pre-release retail)', 'Pre-release Pitch Black ETB in-hand ($49.99 + WA tax ~= $55); set releases 7/17')`;
  console.log(`Pitch Black ETB (#53864): held ${before} -> ${await held(id)}`);
  await sql.end();
}
